          ...
          {(["small","medium","large"] as PetSize[]).map(s => (
            <button key={s} onClick={() => toggleSize(s)}
              className={"px-3 py-1.5 rounded-full text-sm font-medium transition-all " +
                (filters.sizes.includes(s) ? "bg-orange-500 text-white shadow-md" : "bg-gray-100 text-gray-600 hover:bg-gray-200")}>
        ...
        <div className="flex flex-wrap gap-2">
          {Object.entries(CATEGORY_META).map(([key, meta]) => (
            <button key={key} onClick={() => toggleCat(key as Category)}
              className={"px-3 py-1.5 rounded-full text-sm font-medium transition-all " +
                (filters.categories.includes(key as Category) ? "text-white shadow-md" : "bg-gray-100 text-gray-600 hover:bg-gray-200")}
