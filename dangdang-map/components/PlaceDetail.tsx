...
export default function PlaceDetail({ place, onClose }: Props) {
  ...
  const scorePercent = Math.min(100, (place.verificationScore / 100) * 100);
  ...
  return (
    <div className="bg-white rounded-t-3xl shadow-2xl slide-up max-h-[70vh] overflow-y-auto">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              {place.verificationScore >= 50 && (
                <span className="bg-green-100 text-green-700 text-xs font-bold px-2 py-0.5 rounded-full">✓ 인증</span>
              )}
            </div>
      ...
      <div className="p-5 space-y-5">
        {/* 반려견 정책 */}
        <div>
          <h3 className="text-sm font-bold text-gray-800 mb-3 flex items-center gap-1">🐾 반려견 정책</h3>
        ...
        {/* 환경 품질 */}
        <div>
          <h3 className="text-sm font-bold text-gray-800 mb-3 flex items-center gap-1">🌿 환경 정보</h3>
        ...
        {/* 신뢰도 */}
        <div>
          <h3 className="text-sm font-bold text-gray-800 mb-2 flex items-center gap-1">🛡️ 커뮤니티 신뢰도</h3>
          <div className="bg-gray-100 rounded-full h-3 overflow-hidden">
            <div className="h-full rounded-full transition-all duration-500"
              style={{ width: `${scorePercent}%`, backgroundColor: scorePercent >= 70 ? '#22C55E' : scorePercent >= 40 ? '#F97316' : '#EF4444' }} />
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          {place.tags.map(tag => (
            <span key={tag} className="bg-brand-50 text-brand-700 text-xs font-medium px-3 py-1 rounded-full">#{tag}</span>
          ))}
        </div>

        {/* Actions */}
        <div className="flex gap-3">
          <button className="flex-1 bg-brand-500 text-white font-bold py-3 rounded-2xl hover:bg-brand-600 transition-colors shadow-lg shadow-orange-200">
            📝 리뷰 작성
          </button>
          <button className="flex-1 bg-gray-100 text-gray-700 font-bold py-3 rounded-2xl hover:bg-gray-200 transition-colors">
            📍 방문 인증
          </button>
        </div>
      </div>
...
function EnvBadge({ emoji, label }: { emoji: string; label: string }) {
  return (
    <div className="bg-green-50 rounded-xl p-3 text-center">
      ...
      <p className="text-[11px] font-medium text-gray-600 mt-1">{label}</p>
    </div>
  );
}
