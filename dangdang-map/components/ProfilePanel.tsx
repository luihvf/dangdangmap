      ...
      <div className="bg-white rounded-t-3xl sm:rounded-3xl w-full sm:max-w-md slide-up" onClick={e => e.stopPropagation()}>
        <div className="px-5 pt-4 pb-5">
          ...
          <div className="flex items-center gap-4 mb-6">
            <div className="w-16 h-16 rounded-2xl bg-brand-100 flex items-center justify-center text-3xl">🐕</div>
            <div>
              <h2 className="text-lg font-bold text-gray-900">댕댕이 집사</h2>
              <p className="text-sm text-brand-500 font-medium">⭐ 로컬 가이드 Lv.3</p>
              <p className="text-xs text-gray-400">공헌도 1,240점</p>
            </div>
          </div>

          <div className="grid grid-cols-3 gap-3 mb-6">
            <Stat label="리뷰" value="32" />
            <Stat label="제보" value="8" />
            <Stat label="인증" value="45" />
          </div>

          <h3 className="text-sm font-bold text-gray-800 mb-3">🏅 디지털 배지</h3>
          <div className="grid grid-cols-1 gap-2 mb-4">
            {USER_BADGES.map(b => (
              <div key={b.id} className={`flex items-center gap-3 p-3 rounded-xl ${b.earned ? 'bg-brand-50' : 'bg-gray-50 opacity-50'}`}>
