      ...
      <div className="bg-white rounded-t-3xl sm:rounded-3xl w-full sm:max-w-lg max-h-[85vh] overflow-y-auto slide-up" onClick={e => e.stopPropagation()}>
        <div className="sticky top-0 bg-white/90 backdrop-blur-lg px-5 pt-4 pb-3 border-b border-gray-100 z-10">
          ...
          <div className="flex gap-2">
            <button onClick={() => setTab('place')} className={`flex-1 py-2 rounded-xl text-sm font-bold transition-all ${tab === 'place' ? 'bg-brand-500 text-white' : 'bg-gray-100 text-gray-500'}`}>📍 장소 등록</button>
        ...
        <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {Object.entries(CATEGORY_META).map(([k, v]) => (
                    <button key={k} className="px-3 py-2 rounded-xl bg-gray-100 text-sm hover:bg-brand-50 transition-colors">{v.emoji} {v.label}</button>
                  ))}
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {Object.entries(HAZARD_META).map(([k, v]) => (
                    <button key={k} className="py-3 px-3 rounded-xl bg-red-50 text-sm hover:bg-red-100 transition-colors text-left">{v.emoji} {v.label}</button>
                  ))}
                </div>
              </div>
          ...
          <button onClick={() => setSubmitted(true)}
            className={`w-full font-bold py-4 rounded-2xl text-white transition-colors shadow-lg ${tab === 'place' ? 'bg-brand-500 hover:bg-brand-600 shadow-orange-200' : 'bg-red-500 hover:bg-red-600 shadow-red-200'}`}>
            제보하기
          </button>
        </div>
      </div>
...
function InputField({ label, placeholder }: { label: string; placeholder: string }) {
  return (
    <div>
      ...
      <input type="text" placeholder={placeholder} className="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent" />
    </div>
  );
}
