      ...
      <div className="px-5 pt-4 pb-5">
        ...
        <div className="bg-red-50 rounded-2xl p-4 mb-4">
          ...
          <div className="flex items-center gap-4 mt-3 text-xs text-gray-500">
            <span>🕐 {new Date(hazard.reportedAt).toLocaleString('ko-KR')}</span>
            <span>👥 {hazard.confirmCount}명 확인</span>
          </div>
        </div>
        <div className="flex gap-3">
          <button className="flex-1 bg-red-500 text-white font-bold py-3 rounded-2xl hover:bg-red-600 transition-colors">✅ 아직 위험해요</button>
          <button className="flex-1 bg-green-500 text-white font-bold py-3 rounded-2xl hover:bg-green-600 transition-colors">🙆 해결됐어요</button>
        </div>
      </div>
    </div>
  );
