    ...
    <div className="bg-white rounded-t-3xl shadow-2xl slide-up max-h-[70vh] overflow-y-auto">
      ...
      <div className="p-5 space-y-5">
        ...
        <div className="flex gap-3">
          <button className="flex-1 bg-brand-500 text-white font-bold py-3 rounded-2xl hover:bg-brand-600 transition-colors shadow-lg shadow-orange-200">📝 리뷰 작성</button>
...
function PolicyItem({ label, value }: { label: string; value: string }) {
  return (<div className="bg-gray-50 rounded-xl p-3"><p className="text-[11px] text-gray-400 font-medium">{label}</p><p className="text-sm font-semibold text-gray-800 mt-0.5">{value}</p></div>);
}
function EnvBadge({ emoji, label }: { emoji: string; label: string }) {
  return (<div className="bg-green-50 rounded-xl p-3 text-center"><p className="text-xl">{emoji}</p><p className="text-[11px] font-medium text-gray-600 mt-1">{label}</p></div>);
}
